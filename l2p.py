...
def price_problem(pi, dis, customers, capacity, customer_number):
	...
	for i in range(customer_number + 2):
		for j in range(customer_number + 2):
			if vars[i, j].x > 0.5:
				dic[i] = j
				plt.plot((customers[i]['loc'][0],customers[j]['loc'][0]),(customers[i]['loc'][1],customers[j]['loc'][1]))
...
def main(customers, capacity, customer_number,dis):
	...
	cons = rmp.addConstrs(routes[index]['var'] == 1 for index in range(1, customer_number + 1))
	rmp.optimize()

	dual = [cons[i].pi for i in range(1,customer_number+1)]
	sub_obj = []
